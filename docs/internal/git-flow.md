...
| **master** | Production | Live releases only. Tag final versions here (e.g. `v1.0.4`). |
...
3. **Merge into NEXT-RELEASE**  
   Open a PR (or merge) into **NEXT-RELEASE**. Do not merge feature branches directly into STAGING or master.
...
## Preparing a release (NEXT-RELEASE → STAGING → master)
...
2. **Tag the final version** on master (no suffix):
...
When a critical fix is needed on master without waiting for the next full release:
...
3. **Merge back**  
   - Merge **hotfix** into **master** and tag the new version (e.g. `v1.0.3` → `v1.0.4`).
...
```
feature/*  ──►  NEXT-RELEASE  ──►  STAGING (RC tags: v1.0.4-0, -1, …)  ──►  master (v1.0.4)
                   ▲                    ▲                      ▲
                   │                    │                      │
hotfix/*  ────────┴────────────────────┴──────────────────────┘
```

- **Feature work:** always via NEXT-RELEASE, then STAGING, then master.
- **Release candidates:** tagged on STAGING; iterate with new tags (e.g. -0, -1) until QA passes.
- **Final release:** merge STAGING → master and tag `v1.0.4` (no suffix).
- **Hotfixes:** branch from master, merge back to master and NEXT-RELEASE (and optionally STAGING).
